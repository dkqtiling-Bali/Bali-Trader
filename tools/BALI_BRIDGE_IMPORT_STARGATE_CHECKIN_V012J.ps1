...
$report=Join-Path $logs 'BALI_BRIDGE_IMPORT_STARGATE_REPORT_V012J.txt'
function Add($s){ Add-Content -LiteralPath $report -Value $s -Encoding ASCII }
Set-Content -LiteralPath $report -Value @('BALI BRIDGE IMPORT STARGATE REPORT V012J',('Generated: '+(Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS') -Encoding ASCII
