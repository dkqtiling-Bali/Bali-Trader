...
$zip=Join-Path $outDir ('STARGATE_NODE_JOIN_KIT_V012J_'+$stamp+'.zip')
if(Test-Path -LiteralPath $src){
  if(Test-Path -LiteralPath $zip){ Remove-Item -LiteralPath $zip -Force }
  Compress-Archive -Path (Join-Path $src '*') -DestinationPath $zip -Force
}
$report=Join-Path $logs 'BALI_STARGATE_JOIN_KIT_EXPORT_REPORT_V012J.txt'
Set-Content -LiteralPath $report -Encoding ASCII -Value @(
  'BALI STARGATE JOIN KIT EXPORT V012J',
  ('Generated: '+(Get-Date)),
...
ONEDRIVE_BRIDGE_SETUP=READY',
  ...
  ('JOIN_KIT_ZIP='+$zip),
  'RESULT=STARGATE_JOIN_KIT_EXPORT_READY'
)
try { Invoke-Item $outDir | Out-Null } catch {}
