...
Set-Content -LiteralPath $Report -Value @(
  'BALI SPEED LANE UPDATE REPORT V012A',
  ('Generated: ' + (Get-Date)),
...
$updateEngine = Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V012A.ps1'
$updateReport = Join-Path $logs 'BALI_SPEED_LANE_INTERNAL_UPDATE_REPORT_V012A.txt'
...
} else { Add 'WARNING: V012A update engine missing; skipping update step and running checks only.' }
Add ''

Add '=== Step 2: fast health ==='
$healthEngine = Join-Path $rootPath 'tools\BALI_FAST_HEALTH_ENGINE_V012A.ps1'
$healthReport = Join-Path $logs 'BALI_FAST_HEALTH_REPORT_V012A.txt'
...
$iconEngine = Join-Path $rootPath 'tools\BALI_CREATE_FOREVER_DESKTOP_ICON_V012A.ps1'
$iconReport = Join-Path $logs 'BALI_THEMED_DESKTOP_ICON_REPORT_V012A.txt'
...
$cleanupEngine = Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V012A.ps1'
$cleanupReport = Join-Path $logs 'BALI_LAUNCH_BAY_CLEANUP_REPORT_V012A.txt'
...
$statusEngine = Join-Path $rootPath 'tools\BALI_FAST_STATUS_PACK_V012A.ps1'
$statusReport = Join-Path $logs 'BALI_FAST_STATUS_PACK_V012A.txt'
