...
$report=Join-Path $logs 'BALI_ARENA_REFEREE_SCORE_ROUND_REPORT_V012H.txt'
function Add([string]$line){ Add-Content -LiteralPath $report -Value $line -Encoding ASCII }
Set-Content -LiteralPath $report -Value @('BALI ARENA REFEREE SCORE ROUND V012H',('Generated: ' + (Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS','MODE=SIM_ONLY','RAW_LIVE_DATA_ONLY=ON','') -Encoding ASCII
...
$result=[ordered]@{
  version='V012H'; round_id=$roundId; mode='SIM_ONLY'; bridge='SHARED_FOLDER_JSON_ONLY'; raw_live_data_only=$true;
...
$resultPath=Join-Path $resultDir ($roundId + '_V012H.json')
$result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $resultPath -Encoding ASCII
$scoreboard=[ordered]@{
  version='V012H'; mode='SIM_ONLY'; rounds_scored=1; last_round=$roundId; last_winner=$winner;
...
$scoreboard | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $scoreDir 'ARENA_SCOREBOARD_V012H.json') -Encoding ASCII
