...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011W.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V011W.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V011W',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
Set-Content -LiteralPath $report -Value @(
  'BALI TINY UPDATE RESULT V011W',
  ('Generated: ' + (Get-Date)),
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011W.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011W.txt'
