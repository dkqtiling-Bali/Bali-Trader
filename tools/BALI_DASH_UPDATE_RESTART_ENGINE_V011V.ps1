...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011V.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V011V.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V011V',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
Set-Content -LiteralPath $report -Value @(
  'BALI TINY AUTOPILOT RESULT V011V',
  ('Generated: ' + (Get-Date)),
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011V.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011V.txt'
...
Add 'AUTOPILOT=ARMED_LOCAL_ONLY'
Add 'AUTOPILOT_WATCHER=PASS'
Add 'RESULT: PASS'
Add 'NEXT=Drop/upload next patch ZIP; autopilot watcher will apply it from the open dashboard.'
