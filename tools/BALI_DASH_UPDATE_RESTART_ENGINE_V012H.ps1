...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V012H.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V012H.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V012H',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
...
$closeEngine = Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012H.ps1'
$closeReport = Join-Path $logs 'BALI_CLOSE_DASHBOARD_PORT_REPORT_V012H.txt'
$closeOk = 'UNKNOWN'
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V012H.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V012H.txt'
