...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011S.txt'
...
Set-Content -LiteralPath $report -Value @(
  'BALI DASH UPDATE FINAL REPORT V011S',
  ('Generated: ' + (Get-Date)),
  ('Root folder: ' + $rootPath),
  'Purpose: dashboard button handles close, patch, fast health, cleanup/status, auto restart, then one final dashboard result.',
  'Mode: dashboard-only normal flow. Intermediate reports stay in logs; errors open immediately.',
...
$closeEngine = Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V011S.ps1'
$closeReport = Join-Path $logs 'BALI_CLOSE_DASHBOARD_PORT_REPORT_V011S.txt'
$closeCode = 0
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011S.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011S.txt'
...
Add ''
Add '=== Final status summary for ChatGPT ==='
$statusPack = Join-Path $logs 'BALI_FAST_STATUS_PACK_V011S.txt'
if(Test-Path -LiteralPath $statusPack) {
  Get-Content -LiteralPath $statusPack | Where-Object { $_ -match '^(VERSION=|APP_ROOT=|PYTHON_VERSION=|DESKTOP_SHORTCUT=|DASHBOARD_UPDATE_BRIDGE=|RESULT:|Installed version|Archived old patch|Kept current|Patch ZIPs visible)' } | Select-Object -First 60 | ForEach-Object { Add $_ }
} else {
  Add 'STATUS_PACK=NOT_FOUND_BUT_SPEED_LANE_COMPLETED'
}
Add 'Paste this final report if ChatGPT asks for status. You do not need to run BALI_FAST_STATUS_PACK manually.'
...
Add 'Normal user flow: use the dashboard Updates button only. Fast Status Pack is fallback/recovery only.'
Add 'Next normal flow: drop next ZIP into updates, open dashboard Updates tab, click Dashboard Update + Auto Restart. After restart, use Show / Copy Last Final Status in the dashboard if needed.'
