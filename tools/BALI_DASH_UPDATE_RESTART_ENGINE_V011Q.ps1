...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011Q.txt'
...
Set-Content -LiteralPath $report -Value @(
  'BALI DASH UPDATE FINAL REPORT V011Q',
  ('Generated: ' + (Get-Date)),
  ('Root folder: ' + $rootPath),
  'Purpose: close previous dashboard, apply patch, fast health, cleanup/status, auto restart, then one final report.',
...
Add '=== Step 0: close previous dashboard ==='
...
if(Test-Path -LiteralPath $closeEngine -PathType Leaf) {
  ...
  if(Test-Path -LiteralPath $closeReport) {
    Get-Content -LiteralPath $closeReport | Where-Object { $_ -match '^(PORT_STATUS|CLOSED_DASHBOARD_PID|PORT_STATUS_AFTER_CLOSE|Closed count|RESULT:)' } | Select-Object -First 20 | ForEach-Object { Add $_ }
  }
} else {
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011Q.ps1'
if(-not (Test-Path -LiteralPath $speedEngine -PathType Leaf)) {
  $fallback = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011P.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011Q.txt'
...
if(Test-Path -LiteralPath $speedReport) {
  Get-Content -LiteralPath $speedReport | Where-Object { $_ -match '^(Selected version|Selected patch zip|Installed version|Highest available patch|RESULT:|Patch zip|Compact status pack|Desktop shortcut|Archived old patch|Kept current)' } | Select-Object -First 35 | ForEach-Object { Add $_ }
}
