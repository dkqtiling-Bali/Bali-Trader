...
Set-Content -LiteralPath $Report -Value @(
  'BALI SPEED LANE UPDATE REPORT V011Q',
  ('Generated: ' + (Get-Date)),
...
$closeEngine = Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V011Q.ps1'
$closeReport = Join-Path $logs 'BALI_CLOSE_DASHBOARD_PORT_REPORT_V011Q.txt'
if(Test-Path -LiteralPath $closeEngine -PathType Leaf) {
  ...
  if(Test-Path -LiteralPath $closeReport) { Get-Content -LiteralPath $closeReport | Where-Object { $_ -match '^(PORT_STATUS|CLOSED_DASHBOARD_PID|PORT_STATUS_AFTER_CLOSE|RESULT:)' } | Select-Object -First 20 | ForEach-Object { Add $_ } }
} else { Add 'WARNING: close dashboard engine missing; continuing.' }
Add ''

Add '=== Step 1: one-click update engine ==='
$updateEngine = Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V011Q.ps1'
$updateReport = Join-Path $logs 'BALI_SPEED_LANE_INTERNAL_UPDATE_REPORT_V011Q.txt'
...
} else { Add 'WARNING: V011Q update engine missing; skipping update step and running checks only.' }
Add ''

Add '=== Step 2: fast health ==='
$healthEngine = Join-Path $rootPath 'tools\BALI_FAST_HEALTH_ENGINE_V011Q.ps1'
$healthReport = Join-Path $logs 'BALI_FAST_HEALTH_REPORT_V011Q.txt'
...
$iconEngine = Join-Path $rootPath 'tools\BALI_CREATE_FOREVER_DESKTOP_ICON_V011Q.ps1'
$iconReport = Join-Path $logs 'BALI_THEMED_DESKTOP_ICON_REPORT_V011Q.txt'
...
$cleanupEngine = Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V011Q.ps1'
$cleanupReport = Join-Path $logs 'BALI_LAUNCH_BAY_CLEANUP_REPORT_V011Q.txt'
...
$statusEngine = Join-Path $rootPath 'tools\BALI_FAST_STATUS_PACK_V011Q.ps1'
$statusReport = Join-Path $logs 'BALI_FAST_STATUS_PACK_V011Q.txt'
...
Add 'Next normal flow: put next patch ZIP in root/updates, then use dashboard Updates tab: Dashboard Update + Auto Restart.'
Add 'For ChatGPT: paste BALI_FAST_STATUS_PACK_V011Q.txt first; paste this speed-lane report only if there is a warning/fail.'
