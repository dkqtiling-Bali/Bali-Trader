...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V012C.ps1')) { Add 'UPDATE_ENGINE_V012C=FOUND' } else { Add 'UPDATE_ENGINE_V012C=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ROOT_GUARD_V012C.ps1')) { Add 'ROOT_GUARD_V012C=FOUND' } else { Add 'ROOT_GUARD_V012C=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_DASH_UPDATE_RESTART_ENGINE_V012C.ps1')) { Add 'DASH_RESTART_ENGINE_V012C=FOUND' } else { Add 'DASH_RESTART_ENGINE_V012C=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012C.ps1')) { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012C=FOUND' } else { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012C=MISSING' }
Add 'FINAL_REPORT_ONLY=ON'




$iconL = Join-Path $rootPath 'assets\BALI_FOREVER_THEME_ICON_V012C.ico'
...
if(Test-Path -LiteralPath $iconL -PathType Leaf) { Add 'BALI_THEMED_ICON=FOUND_V012C' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V012C.ps1')) { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012C=FOUND' } else { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012C=MISSING' }

Add 'AUTOPILOT_UPDATE=FOUND_V012C'
Add 'IMPORTANT_REPORT_ONLY=ON'
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\referee\REFEREE_SCORING_RULES_V012C.json')){ Add 'MATCH_REFEREE_PREP=FOUND' } else { Add 'MATCH_REFEREE_PREP=MISSING_NON_BLOCKING' }
