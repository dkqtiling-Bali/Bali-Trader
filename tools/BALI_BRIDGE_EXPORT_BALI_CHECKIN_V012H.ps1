...
$report=Join-Path $logs 'BALI_BRIDGE_EXPORT_REPORT_V012H.txt'
$checkin=[ordered]@{
  version='V012H'; cpu_role='BALI_CPU'; bot='Bali Bot'; mode='SIM_ONLY'; round_id='ROUND_0001_BRIDGE_TEST';
...
$out=Join-Path $SharedPath ('BALI_CPU_CHECKIN_' + (Get-Date -Format yyyyMMdd_HHmmss) + '_V012H.json')
$checkin | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $out -Encoding ASCII
Set-Content -LiteralPath $report -Value @('BALI BRIDGE EXPORT REPORT V012H',('Generated: '+(Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS',('BALI_CHECKIN_EXPORTED='+$out),'BRIDGE=SHARED_FOLDER_JSON_ONLY','RESULT=PASS') -Encoding ASCII
