...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V012A.ps1')) { Add 'UPDATE_ENGINE_V012A=FOUND' } else { Add 'UPDATE_ENGINE_V012A=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ROOT_GUARD_V012A.ps1')) { Add 'ROOT_GUARD_V012A=FOUND' } else { Add 'ROOT_GUARD_V012A=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_DASH_UPDATE_RESTART_ENGINE_V012A.ps1')) { Add 'DASH_RESTART_ENGINE_V012A=FOUND' } else { Add 'DASH_RESTART_ENGINE_V012A=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012A.ps1')) { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012A=FOUND' } else { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012A=MISSING' }
Add 'FINAL_REPORT_ONLY=ON'




$iconL = Join-Path $rootPath 'assets\BALI_FOREVER_THEME_ICON_V012A.ico'
...
if(Test-Path -LiteralPath $iconL -PathType Leaf) { Add 'BALI_THEMED_ICON=FOUND_V012A' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V012A.ps1')) { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012A=FOUND' } else { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012A=MISSING' }

Add 'AUTOPILOT_UPDATE=FOUND_V012A'
