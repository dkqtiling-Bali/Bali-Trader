...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V012J.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V012J.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V012J',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
...
$closeEngine = Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012J.ps1'
$closeReport = Join-Path $logs 'BALI_CLOSE_DASHBOARD_PORT_REPORT_V012J.txt'
$closeOk = 'UNKNOWN'
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V012J.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V012J.txt'
...
Add 'CPU_BRIDGE=SHARED_FOLDER_WATCHER_READY_SIM_ONLY'
Add 'MATCH_REFEREE=SCORING_READY_SIM_ONLY'
Add 'BRIDGE=JSON_ONLY_NO_REMOTE_COMMANDS'
Add 'REFEREE_SCOREBOARD=READY_SIM_ONLY'
Add 'RAW_LIVE_DATA_ONLY=ON'
Add 'RAW_DATA_GATE=PASS_REQUIRED_FOR_SCORING'
Add 'BOT_GUARDIAN=ON_SIM_ONLY'
Add 'BOT_HEARTBEATS=ON'
Add 'PHONE_MONITOR=READY_READ_ONLY_LAN'
Add 'GOVERNOR_HUB=BALI_CPU'
Add 'STARGATE_JOIN_KIT=READY'
Add 'STARGATE_JOIN_KIT_EXPORT=READY
ONEDRIVE_BRIDGE_SETUP=READY'
Add 'GOVERNOR_HUB_STATUS=READY'
Add 'STARGATE_ROLE=COMPETITOR_NODE_JSON_CHECKINS_ONLY'
Add 'HUB=GOVERNOR_REFEREE'
Add 'PHONE_CONTROL=OFF_VIEW_ONLY'
Add 'REMOTE_MONITORING=LOCAL_LAN_ONLY'
Add 'RESULT=PASS'
