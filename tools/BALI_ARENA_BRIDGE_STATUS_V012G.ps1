...
$report=Join-Path $logs 'BALI_ARENA_BRIDGE_STATUS_V012G.txt'
...
Set-Content -LiteralPath $report -Value @('BALI ARENA BRIDGE STATUS V012G',('Generated: '+(Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS','BRIDGE=SHARED_FOLDER_READY_SIM_ONLY',('BALI_JSON_COUNT='+(CountJson $bali)),('STARGATE_JSON_COUNT='+(CountJson $gate)),'NETWORK_EXECUTION=OFF','LIVE_ORDERS=OFF','RESULT=PASS') -Encoding ASCII
