...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V012J.ps1')) { Add 'UPDATE_ENGINE_V012J=FOUND' } else { Add 'UPDATE_ENGINE_V012J=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ROOT_GUARD_V012J.ps1')) { Add 'ROOT_GUARD_V012J=FOUND' } else { Add 'ROOT_GUARD_V012J=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_DASH_UPDATE_RESTART_ENGINE_V012J.ps1')) { Add 'DASH_RESTART_ENGINE_V012J=FOUND' } else { Add 'DASH_RESTART_ENGINE_V012J=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012J.ps1')) { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012J=FOUND' } else { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012J=MISSING' }
Add 'FINAL_REPORT_ONLY=ON'




$iconL = Join-Path $rootPath 'assets\BALI_FOREVER_THEME_ICON_V012J.ico'
...
if(Test-Path -LiteralPath $iconL -PathType Leaf) { Add 'BALI_THEMED_ICON=FOUND_V012J' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V012J.ps1')) { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012J=FOUND' } else { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012J=MISSING' }

Add 'AUTOPILOT_UPDATE=FOUND_V012J'
Add 'IMPORTANT_REPORT_ONLY=ON'
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\referee\REFEREE_SCORING_RULES_V012J.json')){ Add 'MATCH_REFEREE_SCOREBOARD=FOUND' } else { Add 'MATCH_REFEREE_SCOREBOARD=MISSING_NON_BLOCKING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\bridge\BRIDGE_CONFIG_SAMPLE_V012J.json')){ Add 'SHARED_FOLDER_BRIDGE=FOUND' } else { Add 'SHARED_FOLDER_BRIDGE=MISSING_NON_BLOCKING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\raw_live_data\RAW_LIVE_DATA_ONLY_POLICY_V012J.json')){ Add 'RAW_LIVE_DATA_POLICY=FOUND' } else { Add 'RAW_LIVE_DATA_POLICY=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_RAW_LIVE_DATA_GATE_V012J.ps1')){ Add 'RAW_LIVE_DATA_GATE=FOUND' } else { Add 'RAW_LIVE_DATA_GATE=MISSING' }
...
# V012J marker
Add 'STARGATE_JOIN_KIT_EXPORT=READY
ONEDRIVE_BRIDGE_SETUP=READY'
Add 'GOVERNOR_HUB_STATUS=READY'
