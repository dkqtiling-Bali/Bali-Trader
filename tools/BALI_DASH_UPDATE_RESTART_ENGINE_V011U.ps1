...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011U.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V011U.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V011U',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
Set-Content -LiteralPath $report -Value @(
  'BALI TINY UPDATE RESULT V011U',
  ('Generated: ' + (Get-Date)),
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011U.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011U.txt'
...
Add 'AUTOPILOT_PROOF=V011U_READY'
Add 'RESULT: PASS'
Add 'NEXT=Drop/upload next patch ZIP; autopilot will apply it from the open dashboard.'
