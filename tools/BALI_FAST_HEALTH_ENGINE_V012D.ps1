...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V012D.ps1')) { Add 'UPDATE_ENGINE_V012D=FOUND' } else { Add 'UPDATE_ENGINE_V012D=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ROOT_GUARD_V012D.ps1')) { Add 'ROOT_GUARD_V012D=FOUND' } else { Add 'ROOT_GUARD_V012D=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_DASH_UPDATE_RESTART_ENGINE_V012D.ps1')) { Add 'DASH_RESTART_ENGINE_V012D=FOUND' } else { Add 'DASH_RESTART_ENGINE_V012D=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V012D.ps1')) { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012D=FOUND' } else { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V012D=MISSING' }
Add 'FINAL_REPORT_ONLY=ON'




$iconL = Join-Path $rootPath 'assets\BALI_FOREVER_THEME_ICON_V012D.ico'
...
if(Test-Path -LiteralPath $iconL -PathType Leaf) { Add 'BALI_THEMED_ICON=FOUND_V012D' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V012D.ps1')) { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012D=FOUND' } else { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V012D=MISSING' }

Add 'AUTOPILOT_UPDATE=FOUND_V012D'
Add 'IMPORTANT_REPORT_ONLY=ON'
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\referee\REFEREE_SCORING_RULES_V012D.json')){ Add 'MATCH_REFEREE_PREP=FOUND' } else { Add 'MATCH_REFEREE_PREP=MISSING_NON_BLOCKING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'game_arena\bridge\BRIDGE_CONFIG_SAMPLE_V012D.json')){ Add 'SHARED_FOLDER_BRIDGE=FOUND' } else { Add 'SHARED_FOLDER_BRIDGE=MISSING_NON_BLOCKING' }
