...
$report = Join-Path $logs 'BALI_ARENA_REFEREE_PREP_REPORT_V012J.txt'
function Add($x){ Add-Content -LiteralPath $report -Value $x -Encoding ASCII }
Set-Content -LiteralPath $report -Value @('BALI ARENA REFEREE PREP REPORT V012J',('Generated: ' + (Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS','') -Encoding ASCII
$rules = Join-Path $RootPath 'game_arena\referee\REFEREE_SCORING_RULES_V012J.json'
$bali = Join-Path $RootPath 'game_arena\bridge\outbox\BALI_CPU_CHECKIN_SAMPLE_V012J.json'
$gate = Join-Path $RootPath 'game_arena\bridge\inbox\STARGATE_CPU_CHECKIN_SAMPLE_V012J.json'
