...
$report=Join-Path $logs 'BALI_ARENA_REFEREE_SCORE_ROUND_REPORT_V012E.txt'
function Add([string]$line){ Add-Content -LiteralPath $report -Value $line -Encoding ASCII }
Set-Content -LiteralPath $report -Value @('BALI ARENA REFEREE SCORE ROUND V012E',('Generated: ' + (Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS','MODE=SIM_ONLY','') -Encoding ASCII
...
function Score($bot){
  ...
  return @{score=$score; safety='PASS'; detail='scored'}
}
...
$result=[ordered]@{
  version='V012E'; round_id=$roundId; mode='SIM_ONLY'; bridge='SHARED_FOLDER_JSON_ONLY';
  ...
  notes='Fun SIM-only referee score. No orders, no API keys, no remote commands.'
}
$resultPath=Join-Path $resultDir ($roundId + '_V012E.json')
$result | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $resultPath -Encoding ASCII
$scoreboard=[ordered]@{
  version='V012E'; mode='SIM_ONLY'; rounds_scored=1; last_round=$roundId; last_winner=$winner;
...
$scoreboard | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $scoreDir 'ARENA_SCOREBOARD_V012E.json') -Encoding ASCII
