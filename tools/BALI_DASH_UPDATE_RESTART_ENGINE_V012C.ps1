...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V012C.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V012C.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V012C',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V012C.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V012C.txt'
