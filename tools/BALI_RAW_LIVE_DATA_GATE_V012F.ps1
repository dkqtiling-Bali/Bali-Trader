...
$report=Join-Path $logs 'BALI_RAW_LIVE_DATA_GATE_REPORT_V012F.txt'
function Add([string]$line){ Add-Content -LiteralPath $report -Value $line -Encoding ASCII }
Set-Content -LiteralPath $report -Value @('BALI RAW LIVE DATA GATE V012F',('Generated: '+(Get-Date)),'SAFETY=LIVE_ORDERS_OFF | CHAMPION_LOCK_LOCKED | NO_API_KEYS','RAW_LIVE_DATA_ONLY=ON') -Encoding ASCII
