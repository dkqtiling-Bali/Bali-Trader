...
$report = Join-Path $logs 'BALI_DASH_UPDATE_FINAL_REPORT_V011X.txt'
$debug = Join-Path $logs 'BALI_DASH_UPDATE_DEBUG_REPORT_V011X.txt'
...
Set-Content -LiteralPath $debug -Value @('BALI DASH UPDATE DEBUG REPORT V011X',('Generated: ' + (Get-Date)),('Root folder: ' + $rootPath),'') -Encoding ASCII
...
$speedEngine = Join-Path $rootPath 'tools\BALI_SPEED_LANE_ENGINE_V011X.ps1'
...
$speedReport = Join-Path $logs 'BALI_DASH_INTERNAL_SPEED_LANE_REPORT_V011X.txt'
...
# Rebuild the success report with the installed version in the title so future patches do not show stale headers.
