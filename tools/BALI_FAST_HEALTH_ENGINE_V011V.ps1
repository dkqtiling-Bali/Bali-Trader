...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V011V.ps1')) { Add 'UPDATE_ENGINE_V011V=FOUND' } else { Add 'UPDATE_ENGINE_V011V=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_ROOT_GUARD_V011V.ps1')) { Add 'ROOT_GUARD_V011V=FOUND' } else { Add 'ROOT_GUARD_V011V=MISSING' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_DASH_UPDATE_RESTART_ENGINE_V011V.ps1')) { Add 'DASH_RESTART_ENGINE_V011V=FOUND' } else { Add 'DASH_RESTART_ENGINE_V011V=MISSING' }
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V011V.ps1')) { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V011V=FOUND' } else { Add 'CLOSE_PREVIOUS_DASHBOARD_ENGINE_V011V=MISSING' }
Add 'FINAL_REPORT_ONLY=ON'




$iconL = Join-Path $rootPath 'assets\BALI_FOREVER_THEME_ICON_V011V.ico'
...
if(Test-Path -LiteralPath $iconL -PathType Leaf) { Add 'BALI_THEMED_ICON=FOUND_V011V' }
...
if(Test-Path -LiteralPath (Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V011V.ps1')) { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V011V=FOUND' } else { Add 'LAUNCH_BAY_CLEANUP_ENGINE_V011V=MISSING' }

Add 'AUTOPILOT_UPDATE=FOUND_V011V'
