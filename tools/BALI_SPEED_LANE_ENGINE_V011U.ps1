...
Set-Content -LiteralPath $Report -Value @(
  'BALI SPEED LANE UPDATE REPORT V011U',
  ('Generated: ' + (Get-Date)),
...
$closeEngine = Join-Path $rootPath 'tools\BALI_CLOSE_DASHBOARD_PORT_V011U.ps1'
$closeReport = Join-Path $logs 'BALI_CLOSE_DASHBOARD_PORT_REPORT_V011U.txt'
...
$updateEngine = Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V011U.ps1'
$updateReport = Join-Path $logs 'BALI_SPEED_LANE_INTERNAL_UPDATE_REPORT_V011U.txt'
...
} else { Add 'WARNING: V011U update engine missing; skipping update step and running checks only.' }
Add ''

Add '=== Step 2: fast health ==='
$healthEngine = Join-Path $rootPath 'tools\BALI_FAST_HEALTH_ENGINE_V011U.ps1'
$healthReport = Join-Path $logs 'BALI_FAST_HEALTH_REPORT_V011U.txt'
...
$iconEngine = Join-Path $rootPath 'tools\BALI_CREATE_FOREVER_DESKTOP_ICON_V011U.ps1'
$iconReport = Join-Path $logs 'BALI_THEMED_DESKTOP_ICON_REPORT_V011U.txt'
...
$cleanupEngine = Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V011U.ps1'
$cleanupReport = Join-Path $logs 'BALI_LAUNCH_BAY_CLEANUP_REPORT_V011U.txt'
...
$statusEngine = Join-Path $rootPath 'tools\BALI_FAST_STATUS_PACK_V011U.ps1'
$statusReport = Join-Path $logs 'BALI_FAST_STATUS_PACK_V011U.txt'
