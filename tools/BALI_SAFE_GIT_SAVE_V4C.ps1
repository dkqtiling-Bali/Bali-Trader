...
Write-Host "=========================================="
Write-Host "        BALI SAFE GIT SAVE V4C"
Write-Host "=========================================="
Write-Host "SAFE_TOOLING_ONLY"
Write-Host "No app.py edit. No trading logic change. No live trading. No keys."
Write-Host ""

$scanScript = Join-Path $Base "tools\BALI_OS_SAFETY_SCAN_V4C.ps1"
if (-not (Test-Path -LiteralPath $scanScript)) { throw "Missing V4C safety scan script: $scanScript" }
...
# Stage docs, tooling, and reports only. Do not stage app.py, env, keys, secrets, or strategy/live-trading files.
$safePaths = @(
  "CONSTITUTION.md",
  "BALI_OS_WORKFLOW.md",
...
foreach ($p in $safePaths) {
  if (Test-Path -LiteralPath (Join-Path $Base $p)) { git add -- $p | Out-Null }
}
...
$blockedStaged = @($staged | Where-Object {
  ...
  $_ -match "wallet" -or
  $_ -match "seed" -or
  $_ -match "live[_-]?orders" -or
  $_ -match "live[_-]?trading"
})
