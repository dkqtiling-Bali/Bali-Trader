...
Set-Content -LiteralPath $Report -Value @(
  'BALI SPEED LANE UPDATE REPORT V011S',
  ('Generated: ' + (Get-Date)),
...
$updateEngine = Join-Path $rootPath 'tools\BALI_ONE_CLICK_UPDATE_ENGINE_V011S.ps1'
$updateReport = Join-Path $logs 'BALI_SPEED_LANE_INTERNAL_UPDATE_REPORT_V011S.txt'
...
} else { Add 'WARNING: V011S update engine missing; skipping update step and running checks only.' }
Add ''

Add '=== Step 2: fast health ==='
$healthEngine = Join-Path $rootPath 'tools\BALI_FAST_HEALTH_ENGINE_V011S.ps1'
$healthReport = Join-Path $logs 'BALI_FAST_HEALTH_REPORT_V011S.txt'
...
$iconEngine = Join-Path $rootPath 'tools\BALI_CREATE_FOREVER_DESKTOP_ICON_V011S.ps1'
$iconReport = Join-Path $logs 'BALI_THEMED_DESKTOP_ICON_REPORT_V011S.txt'
...
$cleanupEngine = Join-Path $rootPath 'tools\BALI_LAUNCH_BAY_CLEANUP_V011S.ps1'
$cleanupReport = Join-Path $logs 'BALI_LAUNCH_BAY_CLEANUP_REPORT_V011S.txt'
...
$statusEngine = Join-Path $rootPath 'tools\BALI_FAST_STATUS_PACK_V011S.ps1'
$statusReport = Join-Path $logs 'BALI_FAST_STATUS_PACK_V011S.txt'
...
Add 'For ChatGPT: paste BALI_FAST_STATUS_PACK_V011S.txt first; paste this speed-lane report only if there is a warning/fail.'
