...
set "GUARD=%LOCAL_PATCH_DIR%\tools\BALI_ROOT_GUARD_V012A.ps1"
if not exist "%GUARD%" (
  echo BALI V012A EXTRACTED PATCH RESCUE
  echo RESULT: ROOT GUARD MISSING
  echo This rescue must be run from an extracted V012A patch folder.
  pause
  exit /b 9
)
...
set "REPORT=%LOGDIR%\BALI_EXTRACTED_PATCH_RESCUE_REPORT_V012A.txt"
(
  echo BALI EXTRACTED PATCH FOLDER RESCUE REPORT V012A
  echo Generated: %DATE% %TIME%
  echo Local extracted patch folder: %LOCAL_PATCH_DIR%
  echo Detected real root: %REAL_ROOT%
  echo Purpose: copy V012A files to the real Bali root if the ZIP was extracted and run from inside updates by mistake.
