...
set "GUARD=%LOCAL_PATCH_DIR%\tools\BALI_ROOT_GUARD_V011P.ps1"
if not exist "%GUARD%" (
  echo BALI V011P EXTRACTED PATCH RESCUE
  echo RESULT: ROOT GUARD MISSING
  echo This rescue must be run from an extracted V011P patch folder.
  pause
  exit /b 9
)
...
set "REPORT=%LOGDIR%\BALI_EXTRACTED_PATCH_RESCUE_REPORT_V011P.txt"
(
  echo BALI EXTRACTED PATCH FOLDER RESCUE REPORT V011P
  echo Generated: %DATE% %TIME%
  echo Local extracted patch folder: %LOCAL_PATCH_DIR%
  echo Detected real root: %REAL_ROOT%
  echo Purpose: copy V011P files to the real Bali root if the ZIP was extracted and run from inside updates by mistake.
