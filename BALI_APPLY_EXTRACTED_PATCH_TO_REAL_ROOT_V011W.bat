...
set "GUARD=%LOCAL_PATCH_DIR%\tools\BALI_ROOT_GUARD_V011W.ps1"
if not exist "%GUARD%" (
  echo BALI V011W EXTRACTED PATCH RESCUE
  echo RESULT: ROOT GUARD MISSING
  echo This rescue must be run from an extracted V011W patch folder.
  pause
  exit /b 9
)
...
set "REPORT=%LOGDIR%\BALI_EXTRACTED_PATCH_RESCUE_REPORT_V011W.txt"
(
  echo BALI EXTRACTED PATCH FOLDER RESCUE REPORT V011W
  echo Generated: %DATE% %TIME%
  echo Local extracted patch folder: %LOCAL_PATCH_DIR%
  echo Detected real root: %REAL_ROOT%
  echo Purpose: copy V011W files to the real Bali root if the ZIP was extracted and run from inside updates by mistake.
